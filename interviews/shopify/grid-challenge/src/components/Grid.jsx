...
export function Grid() {
  const [position, setPosition] = useState({ coords: { x: 0, y: 0 }, heading: 'right' });
  ...
  const moveForward = () => {
    ...
    const shouldTurnRight = crossedX || crossedY;
    const heading = shouldTurnRight ? calculateNextHeading(position.heading) : position.heading;
      ...
      {Array.from({ length: GRID_SIZE }).map((_, i) => (
        <Row key={i}>
          {Array.from({ length: GRID_SIZE }).map((_, j) => (
            <Cell key={j}>
              {position.coords.x === j && position.coords.y === i ? iconDirectionMap[position.heading] : null}
            </Cell>
          ))}
        </Row>
      ))}
